...
document.querySelector("[data-menu-toggle]").addEventListener("click", function () {
    const icon = document.querySelector("[data-icon]"); // ค้นหา <i> ที่มี data-icon
    const navLinks = document.querySelector("[data-nav-links]") || document.querySelector("[data-nav-links-active]");

    if (icon) {
        // สลับค่า data-icon ระหว่าง "0" และ "1"
        icon.setAttribute("data-icon", icon.getAttribute("data-icon") === "0" ? "1" : "0");
    }

    if (navLinks) {
        // สลับ data-nav-links <-> data-nav-links-active
